...
COMMENT ON TABLE data_sources IS
    'The three fictional research providers behind the fact tables. Different '
    'providers use different methodologies, so figures are not strictly '
...
CREATE TABLE content_genre (
    ...
    lifecycle_stage TEXT NOT NULL
        CHECK (lifecycle_stage IN ('growing', 'mature', 'declining')),
    description     TEXT NOT NULL
);

COMMENT ON TABLE content_genre IS
    'The six content categories tracked market-wide. lifecycle_stage is an '
    'analyst judgement about the category, not a computed field.';
...
CREATE TABLE streaming_service (
    ...
    base_price_usd NUMERIC(5,2) NOT NULL CHECK (base_price_usd > 0),
    tier           TEXT NOT NULL
        CHECK (tier IN ('incumbent', 'mid', 'niche', 'entrant'))
);

COMMENT ON TABLE streaming_service IS
    'One row per service. NOTE: parent_company is not unique — Bellweather Media '
    'owns two services whose subscription revenue is reported as a single bundle. '
    'See annual_revenue.is_bundled.';

COMMENT ON COLUMN streaming_service.base_price_usd IS
    'Advertised standard monthly price at the end of the window. Realised revenue '
    'per subscriber differs because of ad-supported tiers and annual discounts.';
...
CREATE TABLE service_genre_catalog (
    ...
    catalog_titles     INTEGER NOT NULL CHECK (catalog_titles >= 0),
    flagship_franchise TEXT,
    PRIMARY KEY (service_id, genre_id)
);

COMMENT ON TABLE service_genre_catalog IS
    'Which genres each service competes in. Only the two incumbents carry all six. '
    'A row here means the service has a catalogue in that genre; it does not '
    'guarantee genre_engagement rows exist for every month (see known_data_gaps).';
...
CREATE TABLE monthly_subscribers (
    service_id      SMALLINT NOT NULL REFERENCES streaming_service (service_id),
    month           DATE NOT NULL,
    subscribers_eom INTEGER NOT NULL CHECK (subscribers_eom >= 0),
    gross_adds      INTEGER NOT NULL CHECK (gross_adds >= 0),
...
COMMENT ON TABLE monthly_subscribers IS
    'SERVICE GRAIN. subscribers_eom = prior month subscribers_eom + gross_adds - '
    'cancellations, exactly, for every row after a service''s first observed month. '
    'The opening balance for January 2024 predates the window and is not stored.';
...
CREATE TABLE genre_engagement (
    ...
    genre_id            SMALLINT NOT NULL REFERENCES content_genre (genre_id),
    month               DATE NOT NULL,
    viewing_hours_m     NUMERIC(10,2) NOT NULL CHECK (viewing_hours_m >= 0),
    engaged_subscribers INTEGER NOT NULL CHECK (engaged_subscribers >= 0),
    genre_share_pct     NUMERIC(5,2) NOT NULL
        CHECK (genre_share_pct >= 0 AND genre_share_pct <= 100),
    source_id           SMALLINT NOT NULL REFERENCES data_sources (source_id),
    PRIMARY KEY (service_id, genre_id, month),
    FOREIGN KEY (service_id, genre_id)
        REFERENCES service_genre_catalog (service_id, genre_id),
    CONSTRAINT genre_engagement_month_is_first_of_month
        CHECK (EXTRACT(DAY FROM month) = 1)
);

COMMENT ON TABLE genre_engagement IS
    'ENGAGEMENT GRAIN. engaged_subscribers counts subscribers who watched anything '
    'in the genre that month. A subscriber who watches three genres is counted in '
    'all three, so these DO NOT sum to monthly_subscribers.subscribers_eom. '
    'genre_share_pct is the genre''s share of that service''s total viewing hours '
    'and sums to 100 per service-month.';
...
CREATE TABLE retention_cohorts (
    ...
    retained_m1_pct   NUMERIC(5,2) NOT NULL CHECK (retained_m1_pct BETWEEN 0 AND 100),
    retained_m3_pct   NUMERIC(5,2) NOT NULL CHECK (retained_m3_pct BETWEEN 0 AND 100),
    retained_m6_pct   NUMERIC(5,2) NOT NULL CHECK (retained_m6_pct BETWEEN 0 AND 100),
    avg_tenure_months NUMERIC(5,1) NOT NULL CHECK (avg_tenure_months > 0),
    source_id         SMALLINT NOT NULL REFERENCES data_sources (source_id),
    PRIMARY KEY (service_id, quarter_start),
    CONSTRAINT retention_cohorts_monotonic
        CHECK (retained_m1_pct > retained_m3_pct
           AND retained_m3_pct > retained_m6_pct)
);

COMMENT ON TABLE retention_cohorts IS
    'Survival of the cohort that joined during the quarter, measured 1, 3 and 6 '
    'months later. Because it tracks JOINERS, it is not the inverse of the '
    'cancellation rate over the whole base.';

-- Grain 3: service x revenue line x year. Dollars, annual only.
CREATE TABLE annual_revenue (
    service_id     SMALLINT NOT NULL REFERENCES streaming_service (service_id),
    revenue_line   TEXT NOT NULL
        CHECK (revenue_line IN ('subscriptions', 'advertising', 'add_ons')),
    fiscal_year    SMALLINT NOT NULL,
    revenue_usd_m  NUMERIC(10,2) NOT NULL CHECK (revenue_usd_m >= 0),
...
COMMENT ON TABLE annual_revenue IS
    'REVENUE-LINE GRAIN, annual only — there is no monthly revenue anywhere. '
    'FY2026 covers January-June 2026 only (half year); do not compare it '
    'like-for-like with FY2024/FY2025.';

COMMENT ON COLUMN annual_revenue.is_bundled IS
    'TRUE means the figure is a COMBINED total for more than one service, repeated '
    'on each participating service''s row. Summing bundled rows double-counts. '
    'Report the figure once, labelled as a bundle; it cannot be split per service.';
...
CREATE TABLE market_events (
    ...
    event_type TEXT NOT NULL CHECK (event_type IN (
        'launch', 'price_change', 'content_deal',
        'franchise_release', 'outage', 'genre_exit')),
...
COMMENT ON TABLE market_events IS
    'Dated events that explain inflections in the fact tables. service_id is NULL '
    'for market-wide events.';
...
COMMENT ON TABLE known_data_gaps IS
    'Deliberate, documented holes. Consult this before concluding that a zero or an '
    'empty result means "no activity" — it may mean "not measured".';

CREATE INDEX idx_monthly_subscribers_month ON monthly_subscribers (month);
CREATE INDEX idx_genre_engagement_month    ON genre_engagement (month);
CREATE INDEX idx_genre_engagement_genre    ON genre_engagement (genre_id, month);
CREATE INDEX idx_annual_revenue_year       ON annual_revenue (fiscal_year);
...
COMMENT ON VIEW v_market_share_by_month IS
    'Service grain. Share is of observed subscriptions, which double-counts people '
    'who subscribe to several services — subscriber overlap is not modelled.';
...
COMMENT ON VIEW v_genre_market IS
    'Engagement grain. Totals cover only services that report engagement in that '
    'month — the three niche services are absent before July 2024, which depresses '
    'early totals. Check known_data_gaps before reading a trend across that break.';
...
COMMENT ON VIEW v_service_overview IS
    'Mixes three grains in one row for convenience. Each column is a LATEST '
    'observation at its own cadence — monthly, quarterly, annual — so never treat '
    'the row as a single consistent period. If subscription_is_bundled is TRUE the '
    'revenue figure is shared with another service and must not be added up.';
...
COMMENT ON VIEW v_competitive_sets IS
    'Catalogue overlap only. It says nothing about whether the two services actually '
    'compete for the same viewers — subscriber overlap is not modelled.';
...
COMMENT ON VIEW v_price_change_impact IS
    'Three months before to four months after each price change. months_from_event '
    '= 0 is the month the new price took effect; the cancellation spike typically '
    'lands at +1 to +2.';
...
COMMENT ON VIEW v_entrant_ramp IS
    'month_index is months since the service was FIRST OBSERVED in this dataset, '
    'which equals months since launch only for the entrant. For services that '
    'launched before January 2024, month_index 1 is simply January 2024.';
...
COMMENT ON VIEW v_revenue_per_subscriber IS
    'revenue_per_subscriber_usd is annual, not monthly. Rows where '
    'any_line_bundled is TRUE overstate the figure, because the bundled revenue '
    'covers two services but is divided by one service''s subscribers. FY2026 '
    'covers six months of revenue, so its figure is roughly half-year.';
...
COMMENT ON VIEW v_genre_competition_trend IS
    'Share of genre viewing hours, per service per month, with the competitor count '
    'alongside. A share that falls while absolute hours rise means the genre grew '
    'faster than the service did.';
